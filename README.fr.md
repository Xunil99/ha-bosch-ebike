...
> [Deutsch](README.md) | [English](README.md#english) | [Nederlands](README.nl.md) | **Français** | [Italiano](README.it.md) | [Español](README.es.md) | [Čeština](README.cs.md)
...
> En plus de l'intégration HACS, ce dépôt contient aussi une **passerelle BLE ESPHome** qui transforme un ESP32 en pont vers le **Bosch eBike Live Data Interface**. Le SoC de la batterie, la vitesse, le compteur kilométrique & co. arrivent ainsi en temps réel dans Home Assistant. Il existe désormais aussi une **passerelle double**, qui relie un seul ESP32 à deux vélos Bosch en même temps.
>
> 🚀 **Flasher sans installer ESPHome** : branche un ESP32 (ou ESP32-C3, par ex. « C3 Mini ») en USB, ouvre dans Chrome / Edge **[https://xunil99.github.io/ha-bosch-ebike/](https://xunil99.github.io/ha-bosch-ebike/)** et clique sur *Install*. L'installateur détecte automatiquement la puce et flashe le firmware adapté (y compris la variante passerelle double). La configuration Wi-Fi se fait dans la même étape du navigateur. Guide complet (DE/EN) avec l'appairage via l'app Flow : [`esphome/`](https://github.com/Xunil99/ha-bosch-ebike/tree/main/esphome).
>
> **Un grand merci à PEPITO82** pour ses tests infatigables de la passerelle double : sans ces tests, la passerelle double ne fonctionnerait toujours pas aujourd'hui, et j'aurais abandonné le projet depuis longtemps.
...
4. Remplis le formulaire :
   ...
   > **Attention, facile à confondre :** les deux champs suivants sont tous les deux des adresses `my.home-assistant.io` qui se ressemblent à première vue. **L'ordre dans le formulaire Bosch peut différer de ce tableau** - saisis chaque valeur dans le champ portant le **nom correspondant**, pas selon la position. Si tu les inverses, tu obtiendras « Invalid parameters are given » en cliquant sur « Service aktivieren », ou « Invalid parameter: redirect_uri » de la part de Bosch lors de l'autorisation dans Home Assistant.

   | Champ dans le formulaire Bosch | Valeur | À quoi ça sert |
   |---|---|---|
   | **Redirect URI** | `https://my.home-assistant.io/redirect/oauth` | Adresse de retour **après** la connexion chez Bosch (callback OAuth) - doit être exactement celle-ci, c'est la redirection officielle « My Home Assistant » grâce à laquelle Home Assistant termine la connexion automatiquement. |
   | **Login URL** | `https://my.home-assistant.io/redirect/config_flow_start/?domain=ha_bosch_ebike` | Lien qu'ouvre **« Service aktivieren »** dans l'eBike Manager pour **démarrer** le flux de configuration directement dans ton instance Home Assistant. |

   > **Remarque :** L'intégration « My Home Assistant » doit être activée dans HA (c'est le cas par défaut). Si tu l'as désactivée, saisis plutôt `https://<ton-URL-HA>/auth/external/callback` pour **Redirect URI**.
