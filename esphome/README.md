...
| **ESP32-C3 never connects to WiFi at all** (works fine with a regular ESP32 board, same network) | Some cheap C3 boards (esp. "ESP32-C3 Super Mini" clones) have weak onboard power regulation and brown out during the current spikes WiFi transmission needs, so the connection attempt silently fails. This is a well-known issue with these boards, not specific to this project. | Build your own YAML from `example-bridge.yaml` plus the `esp32:` block from `factory-c3.yaml`, and add `output_power: 8.5` (and optionally `power_save_mode: none`, already on by default in `factory-c3.yaml`) to the `wifi:` block, then flash it yourself. The precompiled factory firmware intentionally leaves `output_power` uncapped since it reduces range for boards that don't have this problem. |
