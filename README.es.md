...
> [Deutsch](README.md) | [English](README.md#english) | [Nederlands](README.nl.md) | [Français](README.fr.md) | [Italiano](README.it.md) | **Español** | [Čeština](README.cs.md)
...
> Además de la integración HACS, este repositorio contiene un **puente BLE para ESPHome** que convierte un ESP32 en una pasarela hacia el **Bosch eBike Live Data Interface**. Con él, el SoC de la batería, la velocidad, el cuentakilómetros y demás fluyen en tiempo real hacia Home Assistant. Desde hace poco existe además un **puente dual**: un único ESP32 se conecta así simultáneamente a dos eBikes Bosch.
>
> 🚀 **Flashear sin instalar ESPHome**: conecta un ESP32 (o ESP32-C3, p. ej. "C3 Mini") por USB, abre en Chrome / Edge **[https://xunil99.github.io/ha-bosch-ebike/](https://xunil99.github.io/ha-bosch-ebike/)** y pulsa *Install*. El instalador detecta el chip automáticamente y flashea el firmware adecuado (incluida la variante de puente dual). La configuración WiFi se realiza en el mismo paso del navegador. Guía completa (DE/EN) incl. emparejamiento mediante la app Flow: [`esphome/`](https://github.com/Xunil99/ha-bosch-ebike/tree/main/esphome).
>
> **Un enorme agradecimiento a PEPITO82** por sus incansables pruebas del puente dual: sin estas pruebas, el puente dual no funcionaría hoy en día, y yo lo habría abandonado hace tiempo.
...
**Configuración (diferencia respecto al Smart System):** en el portal Bosch Data Act ([portal.bosch-ebike.com/data-act](https://portal.bosch-ebike.com/data-act)), los propietarios de BES2 inician sesión mediante **"Bosch eBike Connect user? Log in here"** (la identidad de eBike Connect), **no** con SingleKey ID, y crean una App / Client-ID como de costumbre. Para conceder el consentimiento de Data Act, el punto de entrada habitual en flow.bosch-ebike.com a menudo no funciona con cuentas eBike Connect - en su lugar necesitas este enlace directo, que te inicia sesión como usuario de eBike Connect y te lleva directamente a la página de Data Act: [flow.bosch-ebike.com/login?returnTo=%2Fdata-act&kc_idp_hint=ebike-connect](https://flow.bosch-ebike.com/login?returnTo=%2Fdata-act&kc_idp_hint=ebike-connect). Activa allí el **interruptor (toggle)** para el cliente creado anteriormente. Al añadir la integración en Home Assistant, elige **eBike System 2** en el **primer paso (selección de sistema)** y luego introduce el Client-ID.
...
- **Intervalo de sondeo de 10 minutos** (en el primer arranque se importan todos los trayectos)
...
4. Rellena el formulario:
   ...
   > **Cuidado, fácil de confundir:** los dos campos siguientes son direcciones `my.home-assistant.io` y se parecen a primera vista. **El orden en el formulario de Bosch puede diferir de esta tabla** - introduce cada valor en el campo con el **nombre correspondiente**, no por posición. Si los intercambias, obtendrás "Invalid parameters are given" al hacer clic en "Service aktivieren", o "Invalid parameter: redirect_uri" de Bosch al autorizar en Home Assistant.

   | Campo en el formulario de Bosch | Valor | Para qué sirve |
   |---|---|---|
   | **Redirect URI** | `https://my.home-assistant.io/redirect/oauth` | Dirección de retorno **después** del inicio de sesión en Bosch (callback OAuth) - debe ser exactamente esta, es la redirección oficial de "My Home Assistant" con la que Home Assistant completa el inicio de sesión automáticamente. |
   | **Login URL** | `https://my.home-assistant.io/redirect/config_flow_start/?domain=ha_bosch_ebike` | Enlace que abre **"Service aktivieren"** en el eBike Manager para **iniciar** el flujo de configuración directamente en tu instancia de Home Assistant. |

   > **Nota:** La integración "My Home Assistant" debe estar activada en HA (lo está por defecto). Si la has desactivado, introduce en **Redirect URI** `https://<tu-URL-de-HA>/auth/external/callback` en su lugar.
...
**Evento de recorrido nuevo (desde la v1.19.36):** la primera vez que un recorrido finalizado aparece en un sondeo se lanza `ha_bosch_ebike_new_activity`, exactamente una vez por recorrido. Los recorridos que ya existían al configurar la integración **no** lo lanzan, a propósito, para que una automatización nueva no se vea inundada por cientos de recorridos antiguos.

El payload es plano y viene ya en las unidades que usan los sensores, de modo que una plantilla no tiene que hacer ningún cálculo:

| Campo | Unidad | Contenido |
|---|---|---|
| `bike_id` | - | La bici a la que se atribuyó el recorrido (puede ser `null` en cuentas con varias bicis) |
| `activity_id`, `title`, `start_time` | - | Identificador, nombre y hora de inicio del recorrido |
| `distance_km` | km | Distancia |
| `duration_min` | min | Tiempo de conducción sin paradas |
| `average_speed`, `max_speed` | km/h | Velocidad media y máxima |
| `elevation_gain` | m | Desnivel acumulado de subida |
| `calories` | kcal | Calorías consumidas |
| `has_tricks` | - | `true` si Bosch proporciona datos de Trick Check para el recorrido |
| `tricks` | - | Los valores completos de Trick Check; si no, `null` |

Cualquier campo puede ser `null` si Bosch no lo proporciona para ese recorrido. Ten en cuenta que Bosch solo publica un recorrido una vez que la app lo ha subido, así que el evento llega cuando el recorrido llega a la nube, no en el momento en que dejas de pedalear.
...
La nube de Bosch no conoce siquiera el concepto de "cargar": solo informa del nivel de batería en el momento de la última sincronización. Sin embargo, quien utiliza el [puente LDI de ESPHome](https://github.com/Xunil99/ha-bosch-ebike/tree/main/esphome) dispone de un **nivel de batería en vivo**, y con eso se puede reconstruir el proceso de carga completo.

Si en las opciones hay un sensor de SoC en vivo vinculado a una bici, para ella se crea el sensor **`Last Charge Energy`** (Wh). Su valor es la energía suministrada en el último proceso de carga completado, calculada a partir del aumento del nivel de batería y de la capacidad de batería configurada. Como atributos están disponibles:

| Atributo | Contenido |
|---|---|
| `start_soc`, `end_soc`, `soc_delta` | Nivel de batería al principio y al final, así como la diferencia (%) |
| `energy_wh` | Energía suministrada en Wh (`null` si no se conoce ninguna capacidad) |
| `duration_min` | Duración del proceso de carga en minutos |
| `started_at`, `ended_at` | Inicio y fin como marcas de tiempo ISO 8601 |
| `signal_gaps` | Cuántas veces falló el sensor en vivo durante la carga |
| `in_progress` | `true` mientras se está cargando |

**Por qué esto resiste los cortes de conexión:** un puente BLE pierde la bici de vez en cuando; ese es el caso normal, no la excepción (véase el issue #68). Por eso, un corte **nunca** da por terminado un proceso de carga; solo se contabiliza en `signal_gaps`. De lo contrario, cada breve salida del alcance de radio se notificaría como una carga completada del 20 %.

Un proceso de carga se considera terminado cuando el nivel de batería baja al menos un 1 % (se vuelve a rodar con la bici) o deja de subir durante 30 minutos (cargador terminado o desenchufado). Siempre se notifica el **valor máximo**, no la última medición: una batería que alcanza el 100 % y después cae al 99 % por autodescarga se cargó al 100 %. Las cargas por debajo del 3 % ni siquiera se publican, para que una recarga breve en el pasillo no sobrescriba la carga real de la noche anterior.
...
Una segunda variante de tarjeta, `bosch-ebike-heatmap-card`, superpone todos los recorridos de una selección como líneas semitransparentes. Desplegables de filtro para el periodo (30 días / 3 meses / 12 meses / todo), cuenta y bici. Debajo, una línea de estado con el número de recorridos y kilómetros de la selección. El estilo del mapa y el aspecto del trazado son configurables, por defecto un azul intenso que resalta bien sobre las carreteras rojas/naranjas del mapa base de OSM.

```yaml
type: custom:bosch-ebike-heatmap-card
height: 600
map_style: topo        # osm (por defecto) / topo / sat
track_color: "#0050ff" # valor por defecto, aquí solo a modo de ejemplo
track_opacity: 0.5     # 0-1
track_weight: 3        # grosor de línea en px
```

Las cuatro opciones son opcionales; sin especificarlas se aplican los valores por defecto indicados arriba.
...
| BES2: se notifica éxito, pero 0 bicis | Activar la compartición de datos mediante el enlace de eBike Connect indicado arriba |
...
| "Invalid parameters are given" al hacer clic en "Service aktivieren", o "Invalid parameter: redirect_uri" de Bosch al autorizar | ¿Has intercambiado Redirect URI y Login URL en el portal de Bosch? Revisa el paso 1 - ambas son direcciones `my.home-assistant.io` y se parecen; cada valor debe ir en el campo con el nombre correspondiente |
