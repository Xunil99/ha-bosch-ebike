...
> Oltre all'integrazione HACS, questo repo contiene anche una **bridge BLE ESPHome** che trasforma un ESP32 in un ponte verso il **Bosch eBike Live Data Interface**. In questo modo SoC della batteria, velocità, chilometraggio & co. arrivano in tempo reale in Home Assistant. Da poco è disponibile anche una **bridge doppia**: un singolo ESP32 si collega così contemporaneamente a due eBike Bosch.
>
> 🚀 **Flash senza installare ESPHome**: collega un ESP32 (o ESP32-C3, ad es. "C3 Mini") via USB, apri in Chrome / Edge **[https://xunil99.github.io/ha-bosch-ebike/](https://xunil99.github.io/ha-bosch-ebike/)** e clicca su *Install*. L'installer riconosce automaticamente il chip e flasha il firmware adatto (inclusa la variante bridge doppia). La configurazione Wi-Fi avviene nello stesso passaggio nel browser. Guida completa (DE/EN) incl. abbinamento tramite l'app Flow: [`esphome/`](https://github.com/Xunil99/ha-bosch-ebike/tree/main/esphome).
>
> **Un grandissimo grazie a PEPITO82** per i test instancabili della bridge doppia: senza questi test, la bridge doppia non funzionerebbe ancora oggi, e avrei abbandonato il progetto da tempo.
...
**Configurazione (differenza rispetto allo Smart System):** nel portale Bosch Data Act ([portal.bosch-ebike.com/data-act](https://portal.bosch-ebike.com/data-act)), i possessori di BES2 accedono tramite **"Bosch eBike Connect user? Log in here"** (l'identità eBike Connect), **non** tramite SingleKey ID, e creano un'App / Client-ID come al solito. Quando aggiungi l'integrazione in Home Assistant, scegli **eBike System 2** nel **primo passaggio (selezione del sistema)** e inserisci poi la Client-ID. Per **concedere l'accesso ai dati**, il normale punto d'ingresso su flow.bosch-ebike.com spesso non funziona per gli account eBike Connect: usa invece questo link diretto, che ti autentica come utente eBike Connect e ti porta alla pagina Data Act corretta: [flow.bosch-ebike.com/login?returnTo=%2Fdata-act&kc_idp_hint=ebike-connect](https://flow.bosch-ebike.com/login?returnTo=%2Fdata-act&kc_idp_hint=ebike-connect). Attiva lì l'**interruttore (toggle)** per il Client creato in precedenza.
...
4. Compila il modulo:
   ...
   > **Attenzione, facile da confondere:** i due campi seguenti sono entrambi indirizzi `my.home-assistant.io` e a prima vista si somigliano. **L'ordine nel modulo Bosch potrebbe differire da questa tabella** - inserisci ogni valore nel campo con il **nome corrispondente**, non in base alla posizione. Se li scambi, otterrai "Invalid parameters are given" cliccando su "Service aktivieren", oppure "Invalid parameter: redirect_uri" da Bosch durante l'autorizzazione in Home Assistant.

   | Campo nel modulo Bosch | Valore | A cosa serve |
   |---|---|---|
   | **Redirect URI** | `https://my.home-assistant.io/redirect/oauth` | Indirizzo di ritorno **dopo** il login su Bosch (callback OAuth) - deve essere esattamente questo, è il redirect ufficiale di "My Home Assistant" tramite cui Home Assistant completa automaticamente il login. |
   | **Login URL** | `https://my.home-assistant.io/redirect/config_flow_start/?domain=ha_bosch_ebike` | Link che **"Service aktivieren"** nell'eBike Manager apre per **avviare** il flusso di configurazione direttamente nella tua istanza Home Assistant. |

   > **Nota:** L'integrazione "My Home Assistant" deve essere attivata in HA (lo è di default). Se l'hai disattivata, inserisci invece `https://<il-tuo-URL-HA>/auth/external/callback` in **Redirect URI**.
...
| BES2: successo segnalato, ma 0 bici | Attivare la condivisione dati tramite il link eBike Connect indicato sopra |
...
| "Invalid parameters are given" cliccando su "Service aktivieren", oppure "Invalid parameter: redirect_uri" da Bosch durante l'autorizzazione | Hai scambiato Redirect URI e Login URL nel portale Bosch? Controlla il passo 1 - sono entrambi indirizzi `my.home-assistant.io` e si somigliano; ogni valore deve stare nel campo con il nome corrispondente |
