...
- **Wartungs-Liste** mit beliebig vielen frei definierbaren Posten (Kette ölen, Kundendienst, Bremsen prüfen, …) – im Editor wählbar aus 11 Vorschlägen oder als freier Text. Pro Posten Trigger über km-Intervall oder Tages-Intervall, plus optionales „zuletzt erledigt"-Datum oder -km. Erscheinen im Dashboard automatisch, sobald sie in den nächsten **500 km** oder **30 Tagen** fällig sind – überfällige Einträge rot, bald fällige gelb, sortiert nach Dringlichkeit. Mit einem grünen Häkchen-Button pro Zeile kannst Du einen Posten direkt als „erledigt" markieren; der Reset wird im Browser-LocalStorage gespeichert und überlebt Reloads (kein Eingriff in die Lovelace-Config nötig, funktioniert daher auch im YAML-Mode).
...
- **Maintenance list** with any number of freely defined tasks (clean chain, major service, check brakes, …) – pick one of 11 presets in the editor or enter free text. Each task triggers by km interval or day interval, with an optional "last done" reference (km or date). The dashboard automatically surfaces items due within the next **500 km** or **30 days**; overdue entries are red, soon-due are amber, sorted by urgency. A green check button on each row marks an item as done; the reset is stored in browser localStorage and survives reloads (no Lovelace-config rewrite required, so it works in YAML mode too).
